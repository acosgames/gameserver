...
const { isObject } = require('shared/util/utils.js');
        ...
        try {
            ...
            for (let action of actions) {
                ...
                if (passed.isGameover || passed.type == "error") {
                    ...
                    let deltaState = delta.delta(previousRoomState, ctx.result, {});
        ...
        if (room.getGameModeName(meta.mode) == "rank" || meta.mode == "rank") {
            ...
            if (ctx.result?.timer?.sequence > 2) {
                if (meta.maxplayers > 1) {
                    ...
                    await room.updateLeaderboard(meta.game_slug, ctx.result.players);
                }
            }
            ...
            if (meta.lbscore || meta.maxplayers == 1) {
                ...
                await room.updateLeaderboardHighscore(meta.game_slug, ctx.result.players);
            }
        }
    }
